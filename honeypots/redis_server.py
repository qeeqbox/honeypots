...
class QRedisServer():
            ...
            def parse_data(self, c, data):
                ...
                if c == 2 or c == 1:
                    ...
                    if username == _q_s.username and password == _q_s.password:
                        ...
                        status = 'success'
                    _q_s.logs.info({'server': 'redis_server', 'action': 'login', 'status': status, 'dest_ip': self.transport.getPeer().host, 'dest_port': self.transport.getPeer().port, 'src_ip': _q_s.ip, 'src_port': _q_s.port, 'username': username, 'password': password})

            def connectionMade(self):
                self._state = 1
                self._variables = {}
                _q_s.logs.info({'server': 'redis_server', 'action': 'connection', 'dest_ip': self.transport.getPeer().host, 'dest_port': self.transport.getPeer().port, 'src_ip': _q_s.ip, 'src_port': _q_s.port})
    ...
    def run_server(self, process=False, auto=False):
        ...
        if process:
            ...
            self.logs.info({'server': 'redis_server', 'action': 'process', 'status': status, 'dest_ip': self.ip, 'dest_port': self.port, 'username': self.username, 'password': self.password, 'src_ip': '0.0.0.0', 'src_port': 0})
