...
'''
            ...
            def render(self, request):
                ...
                _q_s.logs.info({'server': 'http_server', 'action': 'connection', 'dest_ip': request.getHost().host, 'dest_port': request.getHost().port, 'src_ip': _q_s.ip, 'src_port': _q_s.port, 'request': headers})
                ...
                if request.method == b'GET' or request.method == b'POST':
                    _q_s.logs.info({'server': 'http_server', 'action': request.method.decode(), 'dest_ip': request.getHost().host, 'dest_port': request.getHost().port, 'src_ip': _q_s.ip, 'src_port': _q_s.port})
                        ...
                        if _q_s.username != '' and _q_s.password != '':
                            ...
                            if 'username' in form and 'password' in form:
                                ...
                                if username == _q_s.username and password == _q_s.password:
                                    ...
                                    status = 'success'
                                _q_s.logs.info({'server': 'http_server', 'action': 'login', 'status': status, 'dest_ip': request.getHost().host, 'dest_port': request.getHost().port, 'username': username, 'password': password, 'src_ip': _q_s.ip, 'src_port': _q_s.port})
    ...
    def run_server(self, process=False, auto=False):
        ...
        if process:
            ...
            self.logs.info({'server': 'http_server', 'action': 'process', 'status': status, 'dest_ip': self.ip, 'dest_port': self.port, 'username': self.username, 'password': self.password, 'src_ip': '0.0.0.0', 'src_port': 0})
